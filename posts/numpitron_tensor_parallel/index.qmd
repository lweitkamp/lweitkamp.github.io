...
# Compatability with other forms of parallelization
We will discuss the most relevant ones: (fully sharded) data parallel, pipeline parallel, sequence parallel, and more recent, expert parallel. When in doubt, always check Stas Bekman's cheat sheet: [Which Strategy To Use When](https://github.com/stas00/ml-engineering/tree/master/training/model-parallelism#which-strategy-to-use-when).


## (Fully Sharded) Data Parallel
Basic distributed data parallel has no compatability issues with tensor parallelism and is very complementary. With 4 devices and $N_\text{TP}=2$, we would shard the parameters to two devices each, where each of the two devices gets half the global batch size.

Fully Sharded Data Parallel (FSDP) is an implementation of ZeRO-3, the scope of which is out of this blog post. FSDP is also compatible with tensor parallel. The gist of it is that tensor parallel is to sharding computation what FSDP is to sharding parameters. It is a highly optimized way of ensuring minimal memory requirement per GPU and that the model, the optimizer, and the hidden activations are sharded across as many GPUs as permitted.
