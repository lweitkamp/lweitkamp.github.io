...
Because computation is done separately per head, tensor parallel distributes the heads over the devices:

![](attention_tensor_parallel.png)

This ensures that the softmax attention weights are valid. Each device will therefore have in total $4 \times d_\text{model} \times \frac{N_\text{heads}}{2} \times d_\text{head}$ weights in memory (we assume no biases for the attention layer). An `ALLREDUCE`[^7] collects the output per device at the end of the forward pass and also at the end of the backward pass.
...
We un-embed with the same sharded matrix as we embed, which means we will have the outputs logits split into $TP$ chunks. If the other layers only required a smart init upfront (divide head dim or rows or cols by #devices) and some all-reduce terms after, the loss calculation is something else.

First, lets see where we are at after the un-embedding. The figure below depicts the logits after the un-embedding. For each token we have a set of logit predictions of `vocab_size` length.

![](logits.png)

We again have a masking challenge here to ensure the correct label is used per chunk, but we also need to collect the sum of exponentiated logits for a softmax calculation required in the backward pass.


All in all, that's a lot of reductions happening. Thankfully, the length of the messages are quite small () so I guess it was not worth going into detail about it in the Megatron-LM paper.
